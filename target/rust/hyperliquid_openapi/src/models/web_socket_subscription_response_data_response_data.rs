...
#[serde(untagged)]
pub enum WebSocketSubscriptionResponseDataResponseData {
    AllMids(Box<models::AllMids>),
    Notification(Box<models::Notification>),
    WebData2(Box<models::WebData2>),
    Candle(Vec<models::CandleInner>),
    WsBook(Box<models::WsBook>),
    WsTrade(Box<models::WsTrade>),
    WsOrder(Box<models::WsOrder>),
    WsUserEvent(Vec<models::WsUserEventInner>),
    WsUserFill(Box<models::WsUserFill>),
    WsUserFunding(Box<models::WsUserFunding>),
    WsUserNonFundingLedgerUpdate(Box<models::WsUserNonFundingLedgerUpdate>),
}

impl Default for WebSocketSubscriptionResponseDataResponseData {
    fn default() -> Self {
        Self::AllMids(Default::default())
    }
}
