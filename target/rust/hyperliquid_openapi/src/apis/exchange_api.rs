...
pub async fn exchange_post(configuration: &configuration::Configuration, exchange_post_request: models::ExchangePostRequest) -> Result<models::ExchangePost200Response, Error<ExchangePostError>> {
    ...
    let local_var_uri_str = format!("{}/exchange", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());

    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
    local_var_req_builder = local_var_req_builder.json(&exchange_post_request);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ExchangePostError> = serde_json::from_str(&local_var_content).ok();
