...
class ModifyOrderRequestActionOrderTOneOf1Trigger(BaseModel):
    """
    ModifyOrderRequestActionOrderTOneOf1Trigger
    """ # noqa: E501
    is_market: Optional[StrictBool] = Field(default=None, description="Whether the order is a market order.", alias="isMarket")
    trigger_px: Optional[StrictStr] = Field(default=None, description="Trigger price for the order.", alias="triggerPx")
    tpsl: Optional[StrictStr] = Field(default=None, description="Type of trigger, take-profit (tp) or stop-loss (sl).")
    __properties: ClassVar[List[str]] = ["isMarket", "triggerPx", "tpsl"]

    @field_validator('tpsl')
    def tpsl_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['tp', 'sl']):
            raise ValueError("must be one of enum values ('tp', 'sl')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ModifyOrderRequestActionOrderTOneOf1Trigger from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ModifyOrderRequestActionOrderTOneOf1Trigger from a dict"""
        if obj is None:
            return None
