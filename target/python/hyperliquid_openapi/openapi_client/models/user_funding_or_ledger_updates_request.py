...
class UserFundingOrLedgerUpdatesRequest(BaseModel):
    """
    Retrieve a user's funding history or non-funding ledger updates
    """ # noqa: E501
    type: StrictStr = Field(description="The type of ledger updates to retrieve.")
    user: StrictStr = Field(description="Address in 42-character hexadecimal format.")
    start_time: StrictInt = Field(description="Start time in milliseconds, inclusive.", alias="startTime")
    end_time: Optional[StrictInt] = Field(default=None, description="End time in milliseconds, inclusive. Defaults to current time.", alias="endTime")
    __properties: ClassVar[List[str]] = ["type", "user", "startTime", "endTime"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['userFunding', 'userNonFundingLedgerUpdates']):
            raise ValueError("must be one of enum values ('userFunding', 'userNonFundingLedgerUpdates')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of UserFundingOrLedgerUpdatesRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of UserFundingOrLedgerUpdatesRequest from a dict"""
        if obj is None:
            return None
