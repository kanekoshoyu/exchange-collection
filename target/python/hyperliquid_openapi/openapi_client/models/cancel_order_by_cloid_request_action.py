...
class CancelOrderByCloidRequestAction(BaseModel):
    """
    The action object containing details to cancel orders by cloid.
    """ # noqa: E501
    type: Optional[StrictStr] = Field(default=None, description="Must be \"cancelByCloid\".")
    cancels: Optional[List[CancelOrderByCloidRequestActionCancelsInner]] = None
    __properties: ClassVar[List[str]] = ["type", "cancels"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in set(['cancelByCloid']):
            raise ValueError("must be one of enum values ('cancelByCloid')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CancelOrderByCloidRequestAction from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in cancels (list)
        _items = []
        if self.cancels:
            for _item in self.cancels:
                if _item:
                    _items.append(_item.to_dict())
            _dict['cancels'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CancelOrderByCloidRequestAction from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "cancels": [CancelOrderByCloidRequestActionCancelsInner.from_dict(_item) for _item in obj["cancels"]] if obj.get("cancels") is not None else None
        })
        return _obj
