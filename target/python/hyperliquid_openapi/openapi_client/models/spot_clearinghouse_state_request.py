...
class SpotClearinghouseStateRequest(BaseModel):
    """
    ### Request Schema This schema is used to retrieve a user's token balances. 
    """ # noqa: E501
    type: StrictStr = Field(description="The type of request. Use \"spotClearinghouseState\" to retrieve the user's token balances. ")
    user: StrictStr = Field(description="The onchain address in a 42-character hexadecimal format, e.g., 0x0000000000000000000000000000000000000000. ")
    __properties: ClassVar[List[str]] = ["type", "user"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['spotClearinghouseState']):
            raise ValueError("must be one of enum values ('spotClearinghouseState')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SpotClearinghouseStateRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SpotClearinghouseStateRequest from a dict"""
        if obj is None:
            return None
